...
export default function PlayerCharactersTab({ gameId }: { gameId: number }) {
  ...
  const sortedCharacters = sortByCreatedAtDesc(characters)
    ...
    <div className="p-6 max-w-2xl">
      ...
      {characters.length === 0 ? (
        ...
        <div className="grid gap-2">
          {sortedCharacters.map((c: PlayerCharacter) => (
            <div key={c.id} className="bg-gray-800 rounded-xl border border-gray-700 p-4 flex items-center gap-3">
              ...
              <span className="ml-auto text-xs text-gray-500">{formatTimestamp(c.created_at)}</span>
            </div>
          ))}
        </div>
      )}
