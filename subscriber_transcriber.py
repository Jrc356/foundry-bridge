...
speaker_workers: dict[str, SpeakerWorker] = {}
...
async def ensure_speaker_worker(
    participant_id: str,
    label: str,
    sample_rate: int,
    channels: int,
) -> SpeakerWorker:
    existing = speaker_workers.get(participant_id)
    ...
    speaker_workers[participant_id] = worker
    logging.info("Started worker for %s (%s)", label, participant_id)
    return worker


async def close_speaker_worker(participant_id: str) -> None:
    worker = speaker_workers.pop(participant_id, None)
    if worker is None:
        return
...
async def run_bridge_subscriber() -> None:
    pending_audio_header: Optional[dict[str, Any]] = None

    async with connect(BRIDGE_URI, max_size=None) as bridge_ws:
        hello_payload = {
            "type": "hello",
            "role": "subscriber",
            "name": "deepgram-transcriber",
        }
        await bridge_ws.send(json.dumps(hello_payload))
        logging.info("Connected to bridge %s and sent hello: %s", BRIDGE_URI, hello_payload)

        async for message in bridge_ws:
            if isinstance(message, str):
                event = json.loads(message)
                event_type = event.get("type")
                logging.debug("Received JSON event: %s", event)

                if event_type == "audio_frame_header":
                    pending_audio_header = event
                    logging.info("Queued pending audio header for participant %s", event.get("participantId"))
                elif event_type == "ingest_event":
                    ...
                    if inner.get("type") == "participant_detached":
                        ...
                        if participant_id:
                            ...
                            await close_speaker_worker(participant_id)
                else:
                    logging.info("Unhandled JSON event type: %s", event_type)
            else:
                if pending_audio_header is None:
                    logging.warning("Received binary from bridge with no pending header")
                    continue
                ...
                worker = await ensure_speaker_worker(
                    participant_id=participant_id,
                    label=str(label),
                    sample_rate=sample_rate,
                    channels=channels,
                ...
                # Log frame size and worker queue status
                try:
                ...
                logging.info(
                    "Received audio frame: %d bytes for %s (sr=%d ch=%d)",
                    frame_size,
                    label,
                    sample_rate,
                    channels,
                ...
                try:
                    ...
                    qsize = -1
                logging.debug("Worker %s queue size after put: %s", label, qsize)


async def shutdown_all_workers() -> None:
    await asyncio.gather(
        *(close_speaker_worker(pid) for pid in list(speaker_workers.keys())),
